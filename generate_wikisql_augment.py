...
def generate_every_db(db):
    ...
    if "number" in col_types:
        ...
        patterns = load_patterns("data/train_patterns.json", schema)
...
if __name__ == "__main__":
    dbs = json.load(open("data/wikisql_tables.json"))
    count = 0
    data = []
    for db in dbs[:]:
        if count % 1000 == 0:
            print("processed {} files...".format(float(count)/len(dbs)))
        data += generate_every_db(db)
        count += 1
    with open('data/augment_data.json','w') as out_file:
        json.dump(data,out_file)

    print(f"generated {len(data)} new questions")